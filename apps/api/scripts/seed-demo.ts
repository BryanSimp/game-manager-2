...
import { clearDemoCache } from "../src/services/demo.js";
import { createManualGame, upsertGameFromIgdb } from "../src/services/catalog.js";
import { igdbConfigured, searchIgdb } from "../src/services/igdb.js";
 ...
 */
async function resolveGame(title: string, useIgdb: boolean): Promise<string | null> {
  ...
  if (useIgdb) {
    try {
      ...
      console.warn(`  ! IGDB lookup failed for "${title}":`, (err as Error).message);
    }
  }
...
async function main() {
  ...
  for (const entry of LIBRARY) {
    const gameId = await resolveGame(entry.title, useIgdb);
    if (!gameId) continue;
  ...
  console.log(`Library: ${added} games`);
  if (missing.size > 0) {
    console.warn(`  ! platforms not in the seed, skipped: ${[...missing].join(", ")}`);
  }
  ...
  console.log(`Consoles: ${consoleIds.size}`);
  ...
  for (const collection of COLLECTIONS) {
    ...
    for (const [i, title] of titles.entries()) {
      const gameId = gameIds.get(title) ?? (await resolveGame(title, useIgdb));
      if (!gameId) continue;
      ids.push(gameId);
      await db
